...
if(isset($_SESSION['httpClient']))//neu da co client
{
	...
	$loginProvider  = $gdata->getResult();
	$id = $gdata->getresultId();
	$title = $gdata->getResultTitle();
	$_SESSION['LoginProvider']['Google']['Id'] = (string) $id;
	$_SESSION['LoginProvider']['Google']['Title'] = (string) $title;
	
	if($_SESSION['LoginProvider']){	
