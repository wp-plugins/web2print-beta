...
/*
	...
	USAGE:
		...
		$button->accountemail = 'vytien83@yahoo.com';							//the account that is registered with paypal where money will be sent to
		...
		$button->currencycode = 'USD';													//currency code
		...
		$button->image = 'logo.jpg';						//image 150px x 50px that can be displayed on your paypal pages.
		...
		$button->return_url = 'paypal.php';		//url of the page users are sent to after successful payment
		$button->ipn_url = 'ipn.php';		//url of the IPN page (this overrides account settings, IF IPN has been setup at all.
		$button->cancel_url = 'paypal_cancel.php'; //url of the page users are sent to if they cancel through the paypal process
...
class PayPalButton {
	...
	function OutputButton(&$content) {
		if (strlen($this->accountemail) > 0 && (count($this->items) > 0 || count($this->subscriptions) > 0)) {
			//ok
			if (strlen($this->target) > 0) {
				$this->target_text = ' target="' . $this->target . '"';
			}
			$content .= '<form name="frm_payment_vytien" id="frm_payment_vytien" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post"' . $this->target_text . '>' . "\n";
			...
			if (count($this->items) > 0) {
				...
				if (count($this->items) == 1) {
					...
					$content .= '<input type="hidden" name="item_name" value="' . $this->items[0]['name'] . '" />' . "\n";
					$content .= '<input type="hidden" name="quantity" value="' . $this->items[0]['qty'] . '" />' . "\n";
					$content .= '<input type="hidden" name="amount" value="' . $this->items[0]['price'] . '" />' . "\n";
					//item no
					if (strlen($this->items[0]['item_no']) > 0) {
						$content .= '<input type="hidden" name="item_number" value="' . $this->items[0]['item_no'] . '" />' . "\n";
					}
					//shipping
					if (strlen($this->items[0]['shipping']) > 0) {
						$content .= '<input type="hidden" name="shipping" value="' . $this->items[0]['shipping'] . '" />' . "\n";
					}
					//shipping2
					if (strlen($this->items[0]['shipping2']) > 0) {
						$content .= '<input type="hidden" name="shipping2" value="' . $this->items[0]['shipping2'] . '" />' . "\n";
					}
					//handling
					if (strlen($this->items[0]['handling']) > 0) {
						$content .= '<input type="hidden" name="handling" value="' . $this->items[0]['handling'] . '" />' . "\n";
					}
					//first field name
					if (strlen($this->items[0]['firstfieldname']) > 0) {
						$content .= '<input type="hidden" name="on0" value="' . $this->items[0]['firstfieldname'] . '" />' . "\n";
					}
					//first field options
					if (strlen($this->items[0]['firstfieldoptions']) > 0) {
						$content .= '<input type="hidden" name="os0" value="' . $this->items[0]['firstfieldoptions'] . '" />' . "\n";
					}
					//second field name
					if (strlen($this->items[0]['secondfieldname']) > 0) {
						$content .= '<input type="hidden" name="on1" value="' . $this->items[0]['secondfieldname'] . '" />' . "\n";
					}
					//second field options
					if (strlen($this->items[0]['secondfieldoptions']) > 0) {
						$content .= '<input type="hidden" name="os1" value="' . $this->items[0]['secondfieldoptions'] . '" />' . "\n";
					}
					...
					$content .= '<input type="hidden" name="item_name" value="Shopping Cart" />' . "\n";
					$totalamount = 0;
					for ($x=0;$x<count($this->items);$x++) {
						//for each item
						$content .= '<input type="hidden" name="item_name_' . ($x+1) . '" value="' . $this->items[$x]['name'] . '" />' . "\n";
						$content .= '<input type="hidden" name="quantity_' . ($x+1) . '" value="' . $this->items[$x]['qty'] . '" />' . "\n";
						$content .= '<input type="hidden" name="amount_' . ($x+1) . '" value="' . $this->items[$x]['price'] . '" />' . "\n";
						//item no
						if (strlen($this->items[$x]['item_no']) > 0) {
							$content .= '<input type="hidden" name="item_number_' . ($x+1) . '" value="' . $this->items[$x]['item_no'] . '" />' . "\n";
						}
						//shipping
						if (strlen($this->items[$x]['shipping']) > 0) {
							$content .= '<input type="hidden" name="shipping_' . ($x+1) . '" value="' . $this->items[$x]['shipping'] . '" />' . "\n";
						}
						//shipping2
						if (strlen($this->items[$x]['shipping2']) > 0) {
							$content .= '<input type="hidden" name="shipping2_' . ($x+1) . '" value="' . $this->items[$x]['shipping2'] . '" />' . "\n";
						}
						//handling
						if (strlen($this->items[$x]['handling']) > 0) {
							$content .= '<input type="hidden" name="handling_' . ($x+1) . '" value="' . $this->items[$x]['handling'] . '" />' . "\n";
						}
						//tax
						if (strlen($this->items[$x]['tax']) > 0) {
							$content .= '<input type="hidden" name="tax_' . ($x+1) . '" value="' . $this->items[$x]['tax'] . '" />' . "\n";
						}
						//first field name
						if (strlen($this->items[$x]['firstfieldname']) > 0) {
							$content .= '<input type="hidden" name="on0_' . ($x+1) . '" value="' . $this->items[$x]['firstfieldname'] . '" />' . "\n";
						}
						//first field options
						if (strlen($this->items[$x]['firstfieldoptions']) > 0) {
							$content .= '<input type="hidden" name="os0_' . ($x+1) . '" value="' . $this->items[$x]['firstfieldoptions'] . '" />' . "\n";
						}
						//second field name
						if (strlen($this->items[$x]['secondfieldname']) > 0) {
							$content .= '<input type="hidden" name="on1_' . ($x+1) . '" value="' . $this->items[$x]['secondfieldname'] . '" />' . "\n";
						}
						//second field options
						if (strlen($this->items[$x]['secondfieldoptions']) > 0) {
							$content .= '<input type="hidden" name="os1_' . ($x+1) . '" value="' . $this->items[$x]['secondfieldoptions'] . '" />' . "\n";
						}
					...
					$content .= '<input type="hidden" name="amount" value="' . number_format($totalamount,2,".","") . '" />' . "\n";
				}
				//END ITEMS
			...
			//END PURCHASE BUTTON
				...
				$content .= '<input type="hidden" name="modify" value="1" />' . "\n"; //??
				$content .= '<input type="hidden" name="bn" value="PP-SubscriptionsBF" />' . "\n"; //???
				$content .= '<input type="hidden" name="src" value="1" />' . "\n"; //???
				$content .= '<input type="hidden" name="sra" value="1" />' . "\n"; //???
				
				$content .= '<input type="hidden" name="item_name" value="' . $this->subscriptions[0]['name'] . '" />' . "\n";
				$content .= '<input type="hidden" name="a3" value="' . $this->subscriptions[0]['price'] . '" />' . "\n";
				//item no
				if (strlen($this->subscriptions[0]['item_no']) > 0) {
					$content .= '<input type="hidden" name="item_number" value="' . $this->subscriptions[0]['item_no'] . '" />' . "\n";
				}
				//time_period
				if (strlen($this->subscriptions[0]['time_period']) > 0) {
					$content .= '<input type="hidden" name="t3" value="' . $this->subscriptions[0]['time_period'] . '" />' . "\n";
				}
				//interval
				if (strlen($this->subscriptions[0]['interval']) > 0) {
					$content .= '<input type="hidden" name="p3" value="' . $this->subscriptions[0]['interval'] . '" />' . "\n";
				}
			...
			$content .= '<input type="hidden" name="business" value="' . $this->accountemail . '" />' . "\n";
			$content .= '<input type="hidden" name="currency_code" value="' . $this->currencycode . '" />' . "\n";

			//custom
			if (strlen($this->custom) > 0) {
				$content .= '<input type="hidden" name="custom" value="' . $this->custom . '" />' . "\n";
			}
			
			//image
			if (strlen($this->image) > 0) {
				$content .= '<input type="hidden" name="image_url" value="' . $this->image . '" />' . "\n";
			}		
			...
			if (strlen($this->return_url) > 0) {
				$content .= '<input type="hidden" name="return" value="' . $this->return_url . '" />' . "\n";
			}
			//ipn url
			if (strlen($this->ipn_url) > 0) {
				$content .= '<input type="hidden" name="notify_url" value="' . $this->ipn_url . '" />' . "\n";
			}
			//cancel url
			if (strlen($this->cancel_url) > 0) {
				$content .= '<input type="hidden" name="cancel_return" value="' . $this->cancel_url . '" />' . "\n";
			}
			//class css
			...
			if (strlen($this->buttonimage) > 0) {
				$content .= '<input type="image" src="' . $this->buttonimage . '" name="submit" id="submit" alt="' . $this->buttontext . '"' . $class . $width . ' />' . "\n";
			} else {
				$content .= '<input type="submit" value="' . $this->buttontext . '" name="submit" id="submit"' . $class . $width . ' />' . "\n";
			}
			$content .= '</form>';
			return true;
		} else {
			//not ok
			$content .= 'Error: $accountemail directive not set, or no items to sell!';
			return false;
		}
	}
	...
	function OutputSubscriptionCancel() {
		...
		if (strlen($this->accountemail) == 0) {
			...
			echo '<a href="https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_subscr-find&alias=' . $alias . '"' . $target_text . $width . $class . ' tile="' . $this->buttontext . '">' . $button . '</a>';
			return true;
		}
	}
