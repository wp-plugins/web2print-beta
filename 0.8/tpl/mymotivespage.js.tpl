...
dgoMotifAgeRating 	= {motifAgeRating};
dgoResaleUnits		= {resaleUnits};
dgoPaginatorLimit	= {paginatorLimit};
dgoLastCurrentPage  = {lastCurrentPage};
dgoBeginCurrentPage  = {beginCurrentPage};
dgoPageNumber  		 = {pageNumber};
...
function showPaginator(currentPage, pageNumber, option){
	...
	if(option == 'next'){
		...
		dgoLastCurrentPage = end;
		dgoBeginCurrentPage = currentPage;
		
		if(dgoLastCurrentPage == dgoPageNumber){
			paginator = '<div class="paginator-next" style="display:none">'+jQuery('.paginator-next').html()+'</div>';
		}else{
		...
		if(count < dgoPaginatorLimit){
			paginator += '<div class="paginator-number">'+(currentPage - 1)+'</div>';
			dgoBeginCurrentPage = (currentPage - 1);
		}
		
		if(dgoBeginCurrentPage != 1){		
			paginator += '<div class="paginator-more">...</div>';
			paginator += '<div class="paginator-prev" style="display:block">'+jQuery('.paginator-prev').html()+'</div>';			
		}else{
			paginator += '<div class="paginator-prev" style="display:none">'+jQuery('.paginator-prev').html()+'</div>';
		...
		showNewElement(jQuery('.paginator-selected').html() * 1);
	...
	if(option == 'prev'){			
		...
		dgoLastCurrentPage = start;
		dgoBeginCurrentPage = end;
...
function eventForPaginator(){	
	...
	jQuery('.paginator-prev').click(function(){
		if(jQuery('.paginator-selected').next().html() != '...' && isNaN(jQuery('.paginator-selected').next().html()) == false){
			...
			//event for edit button
			eventForEditButton();					
		}else if(jQuery('.paginator-selected').html() != 1){			
			showPaginator(jQuery('.paginator-selected').html() * 1, dgoPageNumber, 'prev');
		}		
	});

	jQuery('.paginator-next').click(function(){		
		if(jQuery('.paginator-selected').prev().html() != '...' && isNaN(jQuery('.paginator-selected').prev().html()) == false){
			...
			//event for edit button
			eventForEditButton();					
		}else if(jQuery('.paginator-selected').html() != dgoPageNumber){			
			showPaginator(jQuery('.paginator-selected').html() * 1, dgoPageNumber, 'next');
		}
	});
...
function eventForEditButton(){
	jQuery(".column-right-editdetails").click(function(){	    	
		...
		//clear all selection sgaring dropdown
		jQuery('#content-dropdown-text').html('...');
		jQuery('.moitf-age-rating').removeClass('motifSharing-selected');
		jQuery('.contentinput').attr('checked','');
		//clear all selection of ageRating dropdown
		...
		if(dgoThumbsArray[id].ThumbnailSetting != undefined){
			...
			for(var i = 0;i < dgoThumbsArray[id].ThumbnailSetting.length;i++){
				...
				if(dgoThumbsArray[id].ThumbnailSetting[i].Key == 'Api.Metadata.AgeRating'){
					jQuery('.popup-share-box').children('.moitf-age-rating').each(function(){
						var sharingSelected = jQuery(this).children('.share-box-ele-input').children('.contentinput').val();							
						if(sharingSelected == dgoThumbsArray[id].ThumbnailSetting[i].Value){
							jQuery(this).click();
						}
					});
				...
				if(i == dgoThumbsArray[id].ThumbnailSetting.length - 1){
					if(jQuery('.motifAgeRating-selected .contentinput').val() == undefined){
						jQuery('#content-dropdown-text').html('...');
						jQuery('.moitf-age-rating').removeClass('motifSharing-selected');
						jQuery('.contentinput').attr('checked','');
					}
...
function showNewElement(currentPage, option){
	...
	jQuery('.search-result-total-val').html(currentPage+'/'+dgoPageNumber);
...
jQuery(function() {
			...
			if(dgoThumbsArray[id].ThumbnailTranslation != undefined){
				...
				if(dgoThumbsArray[id].ThumbnailSetting != undefined){
					...
					if(jQuery('.motifAgeRating-selected .contentinput').val() != undefined){
						for(var i = 0;i < dgoThumbsArray[id].ThumbnailSetting.length;i++){
							if(dgoThumbsArray[id].ThumbnailSetting[i].Key == "Api.Metadata.AgeRating"){
								dgoThumbsArray[id].ThumbnailSetting[i].Value = jQuery('.motifAgeRating-selected .contentinput').val();
								break;
							}else if(i == dgoThumbsArray[id].ThumbnailSetting.length - 1){
								var obj = {
											"Key": "Api.Metadata.AgeRating",
											"Type" : "System.String",
											"Value": jQuery('.motifAgeRating-selected .contentinput').val()
											}
								dgoThumbsArray[id].ThumbnailSetting.push(obj);
							}
						}
	...
	jQuery('.moitf-age-rating').click(function(){
		jQuery('.moitf-age-rating').removeClass('motifAgeRating-selected');
		jQuery(this).addClass('motifAgeRating-selected');			
		jQuery('#content-dropdown-text').html(jQuery(this).children('.share-box-ele-text').html());
		jQuery(this).children('.share-box-ele-input').children('input').attr('checked','checked');
	});   
